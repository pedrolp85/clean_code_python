...
class AwsHcsMockData:
    ...
    def __init__(
        ...
        self.lineitem_resourceid = (
            f"arn:aws:ec2:{aws_generator.region}:instance/i-{aws_generator.resource_id}"
        )
...
identity = {
    ...
    "user": {
        ...
        "is_org_admin": "False",
        "access": {
            "aws.account": {"read": ["8888888888885"]},
            "aws.organizational_unit": {"read": ["QE_OU_002"]},
        },
    },
}
...
hcs_mock_line_item = [
    AwsHcsMockData(
        source, accounts.get("user")[0], accounts.get("payer"), gen, identity, day, month, year
    )
    for gen in data
]
