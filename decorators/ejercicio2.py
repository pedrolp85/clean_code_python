...
def odd_char_to_capital(func: Callable) -> Callable:
    ...
    def wrapper() -> str: 
        initial_string = func()
        odd_list_translated = [ char.upper() if index %2 ==0 else char for index,char in enumerate(initial_string)]
...
def odd_chars_to_b(func: Callable) -> Callable:
    ...
    def wrapper() -> str:
        initial_string = func()
        odd_list_translated = [ f"b" if index %2 ==0 else char for index,char in enumerate(initial_string)]
        return "".join(odd_list_translated)
