...
# Accesos a sockets, ficheros, bdd, que nos queremos asegurar que van a ser
# cerrados después de accedidos
# a pesar de que haya una excepción antes o después del código
# generalmente esto se conseguiría con tun try: finally, pero un context manager
# nos ahorra esta secuencia,
# con los métodos __enter__ y __exit__
import contextlib

# este import es para despues
...
# with llama al método __enter__ y devuelve el return de este método será asignado a la
# variable después de 'as'. Esto es opcional, no hace falta devolver nada
# en el método __enter__ de forma obligatoria

# open es un context manager con una implementación en la librería estándar,
# para gestionar ficheros, pero podemos implementar cualquier context manager

with open(filename) as fd:
    process_file(fd)

# Después del with, podemos ejecutar código de Python,
# con el nuevo contexto, y cuando acaba la última línea de este código,
# se ejecuta el método __exit__
# El método __exit__ se ejecutará siempre,
# aunque haya una excepción o error en código ejecutado en el nuevo contexto.
# De hecho, si ocurre una excepción, el método __exit__
# la recibe por si hay que gestionarla
...
# Para usarlo tenemos que crear una clase que herede de esa
# Base Class, y crear los magic methods
